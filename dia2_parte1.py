...
def get_numero_juego(juego):
    n_juego = ""
    indice = 4
    char = ""
    while char != ":":
        char = juego[indice]
        n_juego = n_juego + char
        indice += 1

    return int(n_juego[:-1])


def get_cantidades_juego(juego):
    max_blue = 0
    max_green = 0
    max_red = 0
    numero_actual = "0"
    for index, char in enumerate(juego):

        if char in numeros:
            numero_actual = numero_actual + char

            if juego[index + 2:index + 6] == "blue":
                max_blue = max(max_blue, int(numero_actual))

            if juego[index + 2:index + 5] == "red":
                max_red = max(max_red, int(numero_actual))

            if juego[index + 2:index + 7] == "green":
                max_green = max(max_green, int(numero_actual))
...
entrada_prueba = ["Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green",
...
suma = 0
for item in entrada:
    num_juego = get_numero_juego(item)
    blue, green, red = get_cantidades_juego(item)

    if blue <= limit_blue and green <= limit_green and red <= limit_red:
        suma += num_juego
